...
origo$year_factor <- factor(origo$year)
# Table 6 adding year fixed effects Logistic Appendix -----
glmo1 <- feglm(fatal~Black + Hispanic + Asian+year_factor ,
               data=origo,
               family=binomial(link="logit"),
               se="cluster",
               cluster="new_group_id")
glmo2 <- feglm(fatal~Black + Hispanic + Asian +city_clean +year_factor,
               data=origo,
               family=binomial(link="logit"),
               se="cluster",
               cluster="new_group_id")
glmo3 <- feglm(fatal~Black + Hispanic + Asian+ trauma10+year_factor  ,
               data=origo,
               family=binomial(link="logit"),
               se="cluster",
               cluster="new_group_id")
...
glmo5 <- feglm(fatal~Black + Hispanic + Asian +trauma10 +
                 city_clean+ officer_race_new+year_factor, 
               data=origo,
               family=binomial(link="logit"),
               se="cluster",
               cluster="new_group_id")
...
# save table
htmlreg(list(glmo1,glmo2,glmo3,glmo4,glmo5),
       custom.coef.map = coefsnames,
       omit.coef = ":",
       include.dev = FALSE,
       include.loglik = FALSE,
       ...
       use.package=FALSE,
       float.pos = "ht!",
       file = here("JOP_racialbias/tables","logistic-FE-models.html"),
...
lpm_main5 <- feols(fatal~Black + Hispanic + Asian +officer_race_new+ trauma10 +
                     city_clean*year_factor,
                   data=origo,
                   se="cluster",
                   cluster=cluster_variable)
...
screenreg(list(lpm_main1,lpm_main2,lpm_main3,lpm_main4,lpm_main5),
          omit.coef = "city|year")
# save table
htmlreg(list(lpm_main1,lpm_main2,lpm_main3,lpm_main4,lpm_main5),
       custom.coef.map = coefsnames,
       omit.coef = ":",
       include.dev = FALSE,
       include.loglik = FALSE,
       ...
       use.package=FALSE,
       float.pos = "ht!",
       file = here("JOP_racialbias/tables","lpm-FE-modelsp.html"),
       label="tab:lpm_FE")
...
pois_main4 <- fepois(fatal~Black + Hispanic + Asian + trauma10 +
                     ...
                     se="cluster",
                     cluster=cluster_variable)
pois_main5 <- fepois(fatal~Black + Hispanic + Asian +officer_race_new + trauma10 +
                       city_clean*year_factor,
                     data=origo,
                     se="cluster",
                     cluster=cluster_variable)
...
screenreg(list(pois_main1,pois_main2,pois_main3,pois_main4,pois_main5),
          omit.coef=c("city|year"))
# save table
htmlreg(list(pois_main1,pois_main2,pois_main3,pois_main4,pois_main5),
       custom.coef.map = coefsnames,
       omit.coef = ":",
       include.dev = FALSE,
       include.loglik = FALSE,
       ...
       use.package=FALSE,
       float.pos = "ht!",
       file = here("JOP_racialbias/tables","poisson-FE-modelsp.html"),
...
temp <- origo %>% filter(!is.na(trauma10))
dim(temp)
length(unique(temp$new_group_id))
length(unique(origo$new_group_id)) # unique incidents
table(origo$fatal)
608/(666+608)
table(origo$city_clean)
