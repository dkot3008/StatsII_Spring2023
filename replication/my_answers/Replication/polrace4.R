...
library(here)
### functions 
source(here('JOP_racialbias','polracs function.R'))
...
# use scaled distance
df <- origo %>% filter(White == 1 | Black == 1) %>%
  select(fatal, White, Black, Houston, King_County, Los_Angeles, Orlando,San_Jose,
         Seattle, Tucson, Charlotte, trauma10,officer_race_new)# with intercept, Charlotte as ref
...
df <- orig_bl_wh %>% 
  select(fatal, White, Black, Houston, King_County, Los_Angeles, Orlando,San_Jose,
         Seattle, Tucson, Charlotte, trauma10, city_factor, new_group_id,officer_race_new) %>%
  na.omit()
...
pois_main <- glm(fatal ~ White +
                   ...
                   San_Jose + Seattle  + Tucson + trauma10+ officer_race_new,
                 data = df, 
...
set.seed(5463)
pois_fe2 <- fepois(fatal ~ White + trauma10 + city_factor+officer_race_new,
                   data = df,
                   se = "cluster",
                   cluster = cluster_variable)

# TO RUN BOOTSTRAP UNCOMMENT THIS CODE --------
# resultsp <- boot(data = df,
 #                 statistic = cluster.pois2,
  #                R = 30000,
   #               parallel = "multicore")
 
 
 #save(resultsp, file=here("JOP_racialbias/data","poisson_boostrappp.RData"))

# IF DO NOT WANT TO RERUN, THIS LOADS SAVED BOOTSTRAP RESULTS ------
load(here("JOP_racialbias/data","poisson_boostrappp.RData"))
boot_results <- resultsp$t
